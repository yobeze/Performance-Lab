...
double applyFilter(class Filter *filter, cs1300bmp *input, cs1300bmp *output)
{
    ...
    //Performing some calculations locally rather than using computing repeatedly
    int w = output -> width = local -> width;
    int h = output -> height = local -> height;
    w = w - 1;
    h = h - 1;
    //size is constant
    const short fooSize = filter -> getSize();
    int filterArr[fooSize][fooSize];
    
    //array of filters locally stored
    //holds x, y values
    //processes and saves the values that were being multiplied inside main loop
    #pragma omp parallel for
    for(short i = 0; i < fooSize; i++)
    {
        for(short j = 0; j < fooSize; j++)
        {
            filterArr[i][j] = filter -> get(i, j);
        }
    }
    //eliminates need for calling divisor repeatedly
    ...
    for(int row = 1; row < h; row++) 
    {
      for(int col = 1; col < w; col++) 
      {
        ...
        { //getSize will always be 3
            for (int i = 0; i < 3; i++) 
            {
                //replaces need for filter -> get(i,j)
                //storing filter in xy vs accessing/computing it yields more efficient
                short xy = filterArr[i][j];
                output -> color[0][row][col] = output -> color[0][row][col] + (local -> color[0][row + i - 1][col + j - 1] * xy);
                output -> color[1][row][col] = output -> color[1][row][col] + (local -> color[1][row + i - 1][col + j - 1] * xy);
                output -> color[2][row][col] = output -> color[2][row][col] + (local -> color[2][row + i - 1][col + j - 1] * xy);
            }
        } 
